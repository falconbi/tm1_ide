  ...
  },
  {
    ...
    patterns: [
      ...
      },
      {
        ...
        description: 'Copy a String attribute into an Alias-type attribute on every element. Same-name (in-place) conversion is also supported via a temp attribute staging pattern. AttrInsert/AttrDelete confirmed IBM PA 2.0 docs.',
        fields: [
        ...
        generate: ({ dim, srcAttr, aliasAttr, handleBlank }) => {
          ...
          const sameNameBranch = lines(
              ...
              `        IF(vVal @= '' & pHandleBlank = 1); vVal = vEl; ENDIF;`,
              `        AttrPutS(vVal, pDimension, vEl, vTempAttr);`,
              `        nIdx = nIdx + 1;`,
              `    END;`,
              ...
              `    WHILE(nIdx <= nCount);`,
              `        vEl = DimNm(pDimension, nIdx);`,
              `        AttrPutS(ATTRS(pDimension, vEl, vTempAttr), pDimension, vEl, pAliasAttrName);`,
              `        nIdx = nIdx + 1;`,
              `    END;`,
              `    AttrDelete(pDimension, vTempAttr);`,
              `    LogOutput('INFO', cProcName | ': Converted ' | pSourceAttr | ' to Alias - ' | NumberToString(nCount) | ' elements');`,
          )
          const diffNameBranch = lines(
              ...
              `        IF(vAlias @<> '');`,
              `            AttrPutS(vAlias, pDimension, vEl, pAliasAttrName);`,
              `            nUpdated = nUpdated + 1;`,
              `        ELSEIF(pHandleBlank = 1);`,
              `            AttrPutS(vEl, pDimension, vEl, pAliasAttrName);`,
              `            nUpdated = nUpdated + 1;`,
              `            LogOutput('WARN', cProcName | ': ' | vEl | ' - blank source, used element name');`,
              `        ELSE;`,
              `            nSkipped = nSkipped + 1;`,
              `            LogOutput('WARN', cProcName | ': ' | vEl | ' - blank source, skipped');`,
              `        ENDIF;`,
              `        nIdx = nIdx + 1;`,
              `    END;`,
              `    LogOutput('INFO', cProcName | ': Done - ' | NumberToString(nUpdated) | ' updated, ' | NumberToString(nSkipped) | ' skipped');`,
          )
          return {
            PrologProcedure: lines(
              `cProcName = GetProcessName();`,
              ``,
              `# Parameters - move to Parameters tab`,
              `pDimension     = '${cDim}';`,
              `pSourceAttr    = '${cSrc}';`,
              `pAliasAttrName = '${cAlias}';`,
              `pHandleBlank   = ${skip ? '0' : '1'};`,
              ``,
              `IF(pDimension @= '' % pSourceAttr @= '' % pAliasAttrName @= '');`,
              `    LogOutput('ERROR', cProcName | ': pDimension, pSourceAttr and pAliasAttrName are all required');`,
              `    ProcessQuit;`,
              `ENDIF;`,
              ``,
              ...
              `ENDIF;`,
              ``,
              `# NOTE: Does not detect duplicate alias values. Verify uniqueness after running.`,
            ),
          }
        },
      },
