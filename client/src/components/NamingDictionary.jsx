...
export default function NamingDictionary({ open, onClose }) {
  ...
  const addEntry = () => {
    ...
    setState(prev => ({
      ...prev,
      rows: [...prev.rows, { id, input: '', output: '', source: 'custom', type: 'custom' }],
    }))
    setSearch('')
    setTypeFilter('all')
      ...
      <div className="bg-card border border-border rounded-lg shadow-lg w-[680px] max-w-[92vw] h-[600px] max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border shrink-0">
          ...
          <div className="flex items-center gap-1">
            ...
            <button onClick={onClose} className="p-1.5 rounded hover:bg-muted text-muted-foreground ml-1"><X size={14} /></button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 px-4 py-2 border-b border-border shrink-0">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search…"
              className="w-full pl-6 pr-2 py-1 text-xs bg-background border border-border rounded outline-none focus:ring-1 focus:ring-ring"
            />
          </div>
